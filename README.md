...
## Running the Server
The instructions below for starting the server locally don't work in
my case.  I use the following in the repository root:
```sh
bundle exec jekyll serve
```

After a recent windows update, I got a ```Can't find gem bundler (>=0a)
with executable bundle``` error.  Fixed with:
```sh
...
To work on this code, create a new branch, make the changes and test
there, then merge that branch into published, and if no longer needed,
delete the branch.
